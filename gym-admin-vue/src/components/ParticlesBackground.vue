<template>
  <div ref="containerRef" :class="[className, 'particles-container']"></div>
</template>
...
const props = defineProps({
  ...
  particleColors: { type: Array, default: () => ['#87CEEB', '#B0E0E6', '#AFEEEE', '#E0FFFF'] },
  ...
  particleBaseSize: { type: Number, default: 180 },
...
const defaultColors = ['#87CEEB', '#B0E0E6', '#AFEEEE', '#E0FFFF'];
...
const fragment = /* glsl */ `
  ...
  void main() {
    ...
    float flicker = 0.3 + 0.2 * sin(uTime * 5.0 + vRandom.y * 6.28 + uv.x * 10.0);
    
    if(uAlphaParticles < 0.5) {
      if(d > 0.5) {
        discard;
      }
      vec3 finalColor = vColor * (1.0 + glow * 0.5 + flicker);
...
const initParticles = () => {
  const container = containerRef.value;
  if (!container) {
    console.log('ParticlesBackground: container is null');
    return;
  }
  
  console.log('ParticlesBackground: container found', container);
  console.log('ParticlesBackground: container dimensions', container.offsetWidth, container.offsetHeight);
...
onMounted(async () => {
  await nextTick();
  console.log('ParticlesBackground: onMounted called');
  console.log('ParticlesBackground: containerRef', containerRef.value);
  initParticles(); 
});
